...
export function PollChart({ poll, type = 'bar', showPercentage = true, animate = true }: PollChartProps) {
  ...
  const commonOptions = {
    ...
    animation: animate ? { duration: 500, easing: 'easeOutQuart' as const } : false as const,
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          label: (context: { raw: number }) => {
            ...
            return showPercentage ? `${value} vote${value !== 1 ? 's' : ''} (${percentage}%)` : `${value} vote${value !== 1 ? 's' : ''}`;
          },
        },
      },
    },
  ...
  const doughnutOptions = { ...commonOptions, cutout: '60%' };
  ...
  return (
    <div className="w-full" role="img" aria-label={`Graphique des résultats`}>
      ...
      <div className="h-64 md:h-80">
        {type === 'bar' ? <Bar data={chartData} options={barOptions} /> : <Doughnut data={chartData} options={doughnutOptions} />}
      </div>
